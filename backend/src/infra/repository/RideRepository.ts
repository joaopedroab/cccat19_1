...
export class RideRepositoryDatabase implements RideRepository  {
	...
	async hasActiveRideByPassengerId(passengerId: string) {
		const ridesData = await this.connection.query("SELECT count(*)::int as count FROM ccca.ride WHERE passenger_id = $1 and status not in ('completed', 'cancelled') limit 1", [passengerId]);
